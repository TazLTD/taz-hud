...
RegisterServerEvent('taz-hud:get:data', function()
	local scr = source
    ...
    for k, v in pairs(huddata) do
        if v.identifier == identifier then
            ...
            TriggerClientEvent('taz-hud:get:data', scr, data)
    ...
    TriggerClientEvent('taz-hud:get:data', scr, defaultdata)
end)

RegisterServerEvent('taz-hud:update', function(settings)
    local scr = source
    local huddata = json.decode(LoadResourceFile(GetCurrentResourceName(), "./huddata.json"))
    local identifier = GetPlayerIdentifiers(scr)[1]
