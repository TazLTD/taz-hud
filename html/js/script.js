...
(() => {
  ...
  TAZ_HUD.ToggleSeatbelt = function (data) {
    if (data.toggle) {
  ...
  TAZ_HUD.UpdateHud = function (data) {
    var Show = "none";
  ...
  TAZ_HUD.UpdateProximity = function (data) {
    let state = 100;
    ...
    window.addEventListener("message", function (event) {
      switch (event.data.action) {
        case "updateStatusHud":
          TAZ_HUD.UpdateHud(event.data);
          break;
        case "adjust":
          TAZ_HUD.UpdateHud(event.data);
          break;
        case "seatbelt":
          TAZ_HUD.ToggleSeatbelt(event.data);
          break;
        case "car":
          TAZ_HUD.CarHud(event.data);
          break;
        case "UpdateProximity":
          TAZ_HUD.UpdateProximity(event.data);
          break;
        case "air":
          if (event.data.show) {
            $(".air-alt").show();
          ...
          break;
        case "hudmenu":
          TAZ_HUD.HudMenuSetupSettings(event.data.settings);
          TAZ_HUD.DisplayHudSettingsMenu(event.data.show);
          break;
        case "settings":
          TAZ_HUD.HudMenuSetupSettings(event.data.settings);
          break;
...
TAZ_HUD.HudMenuSetupSettings = function (settings) {
...
TAZ_HUD.DisplayHudSettingsMenu = function (show) {
  if (show) {
...
$(document).on("keydown", function () {
  ...
  switch (event.keyCode) {
    case 27: // ESC
      TAZ_HUD.DisplayHudSettingsMenu(false);
      $.post("https://taz-hud/closeui");
      break;
  }
});
